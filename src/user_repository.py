...
class User_repository:
    ...
    def ad_user(self,user_dto:UserDTO):
        ...
        if user_dto.role_id==2:
             self.insert_record.add_record("users",["first_name","last_name","email","password","role_id"],[user_dto.first_name,user_dto.last_name,user_dto.email,user_dto.password,user_dto.role_id])

    def get_user_by_email_password(self,email,password)->UserDTO:
    ...
    def get_all_vacations(self)-> list[VacationDTO]:
        ...
        try:
            self.db_connect.cursor.execute("SELECT id, id_country, vacation_description, start_date, end_date, file_image, price FROM vacation ORDER BY start_date ASC;")
    ...
    def add_vacation(self,vacation_dto:VacationDTO):
        try:
            self.insert_record.add_record("vacation",
                ["country_id", "description", "start_date", "end_date", "file_img", "price"],
